...
static void LoadRGB(char *FileName,
    ...
    if ((*RedBuffer = (GifByteType *) malloc((unsigned int) Size)) == NULL ||
        ...
        (*BlueBuffer = (GifByteType *) malloc((unsigned int) Size)) == NULL) GIF_EXIT(
    ...
    if (FileName != NULL) {
        if (OneFileFlag) {
            if ((rgbfp[0] = fopen(FileName, "rb")) == NULL) GIF_EXIT("Can't open input file name.");
        } else {
            ...
            for (i = 0; i < 3; i++) {
                ...
                if ((rgbfp[i] = fopen(OneFileName, "rb")) == NULL) GIF_EXIT(
    ...
    if (OneFileFlag) {
        ...
        if ((Buffer = (GifByteType *) malloc(Width * 3)) == NULL) GIF_EXIT(
                "Failed to allocate memory required, aborted.");

        for (i = 0; i < Height; i++) {
            int j;
            GifQprintf("\b\b\b\b%-4d", i);
            if (fread(Buffer, Width * 3, 1, rgbfp[0]) != 1) GIF_EXIT(
        ...
        for (i = 0; i < Height; i++) {
            ...
            if (fread(RedP, Width, 1, rgbfp[0]) != 1 ||
                ...
                fread(BlueP, Width, 1, rgbfp[2]) != 1) GIF_EXIT(
                    "Input file(s) terminated prematurly.");
            RedP += Width;
            GreenP += Width;
            BlueP += Width;
...
static void RGB2GIF(bool OneFileFlag, int NumFiles, char *FileName,
    ...
    if ((OutputColorMap = GifMakeMapObject(ColorMapSize, NULL)) == NULL ||
        (OutputBuffer = (GifByteType *) malloc(Width * Height *
                                               sizeof(GifByteType))) == NULL) GIF_EXIT(
            "Failed to allocate memory required, aborted.");
...
static void DumpScreen2RGB(char *FileName, int OneFileFlag,
    ...
    if (FileName != NULL) {
        if (OneFileFlag) {
            if ((rgbfp[0] = fopen(FileName, "wb")) == NULL) GIF_EXIT("Can't open input file name.");
        } else {
            static char *Postfixes[] = {".R", ".G", ".B"};
            char OneFileName[80];
    ...
    if (OneFileFlag) {
        ...
        if ((Buffer = (unsigned char *) malloc(ScreenWidth * 3)) == NULL) GIF_EXIT(
                "Failed to allocate memory required, aborted.");
        for (i = 0; i < ScreenHeight; i++) {
            ...
            if (fwrite(Buffer, ScreenWidth * 3, 1, rgbfp[0]) != 1) GIF_EXIT(
        ...
        if ((Buffers[0] = (unsigned char *) malloc(ScreenWidth)) == NULL ||
            ...
            (Buffers[2] = (unsigned char *) malloc(ScreenWidth)) == NULL) GIF_EXIT(
                "Failed to allocate memory required, aborted.");
        ...
        for (i = 0; i < ScreenHeight; i++) {
            ...
            if (fwrite(Buffers[0], ScreenWidth, 1, rgbfp[0]) != 1 ||
                ...
                fwrite(Buffers[2], ScreenWidth, 1, rgbfp[2]) != 1) GIF_EXIT(
                    "Write to file(s) failed.");
        }
...
static void GIF2RGB(int NumFiles, char *FileName,
    ...
    if ((ScreenBuffer = (GifRowType *)
            malloc(GifFile->SHeight * sizeof(GifRowType))) == NULL) GIF_EXIT(
            "Failed to allocate memory required, aborted.");
    ...
    for (i = 1; i < GifFile->SHeight; i++) {
        ...
        if ((ScreenBuffer[i] = (GifRowType) malloc(Size)) == NULL) GIF_EXIT(
                "Failed to allocate memory required, aborted.");
