...
static ColorMapObject *ModifyColorMap(ColorMapObject *ColorMap) {
    ...
    if (SaveFlag) {
        ...
        for (i = 0; i < ColorMap->ColorCount; i++) {
            if (feof(ColorFile)) GIF_EXIT("Color file to load color map from, too small.");
        ...
        for (i = 0; i < ColorMap->ColorCount; i++) {
            int tmp;
            if (feof(TranslateFile)) GIF_EXIT("Color file to load color map from, too small.");
        ...
        if ((NewMap = GifMakeMapObject(1 << GifBitSize(Max + 1), NULL)) == NULL) GIF_EXIT(
                "Out of memory while allocating color map!");
