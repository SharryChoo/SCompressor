    ...
    while (fgets(buf, sizeof(buf), txtin) != (char *) NULL) {
        ...
        if (sscanf(buf, "screen width %d\n", &GifFileOut->SWidth) == 1)
            ...
            if ((DoTranslation = (GifFileOut->SColorMap != (ColorMapObject *) NULL))) {
                ...
                //cppcheck-suppress nullPointerRedundantCheck
                UnionMap = GifUnionColorMap(GifFileOut->SColorMap, Inclusion->SColorMap,
                                            Translation);
            ...
            while (fgets(buf, sizeof(buf), txtin) != (char *) NULL)
                if (strcmp(buf, "end\n") == 0)
                    ...
                    if (GifAddExtensionBlock(&LeadingExtensionBlockCount,
                                             ...
                                             bc++ == CONTINUE_EXT_FUNC_CODE ? COMMENT_EXT_FUNC_CODE
                                                                            : 0,
                                             Len,
            ...
            if ((Raster = (GifPixelType *) malloc(
                    sizeof(GifPixelType) * NewImage->ImageDesc.Width * NewImage->ImageDesc.Height))
                == NULL) {
